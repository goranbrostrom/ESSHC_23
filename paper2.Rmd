...
plot(x[1:4], g(x[1:4]), type = "l", xlab = "Days", ylab = "g(Days)", 
     col = "blue", axes = FALSE)
att <- c(0, 28, 183, 365)
...
```{r third, fig.cap = "The hazards method plotted in the Bourgeous-Pichat way."}
oldpar <- par(mfrow = c(1, 2))
...
It is a consequence of the "paper, pencil and ruler" method used by @bp52a, and it can be
...
Given that the exogeneous mortality follows an Exponential distribution on the log-cube scale, it follows that the cumulative hazards function on the natural time scale is

\begin{equation*}
H(t; \lambda) = \lambda \log^3(t + 1), \; t > 0,
\end{equation*}

and it follows that the hazard function $h$ is given by

\begin{equation*}
h(t; \lambda) = \frac{3 \log^2(t + 1)}{t + 1}, \; t > 0,
\end{equation*}
...
The Scanian dataset covers five parishes in Scania 1710--1800 [@sedd].
...
-->
Apparently, it works for many places in the mid-twentieth century, but beyond that, it is not generalizable. In fact, also when post-neonatal mortality is following a straight line on the log-cube scale, mortality may well deviate during different parts of the year. This means that the methods should be used with great care and only as a starting point.
