...
Bourgeois-Pichat also found deviations from the linear death pattern after the first month, the curve for Sardinia in 1948 bend strongly upward after four months [@bp52, Figure 12], and in the case of Quebec 1944--1947, the curve bends down quite strongly after the sixth month [@bp52, Figure 8]. A downward bend is also what has been found using historical Swedish parish data [@tb99, Figure 6b; @tbcl99, Figures 6 and 7; @lyn98, Figures 1 and 2; @jslgt81, Figure 6d], though less pronounced than in Quebec. @knoki77 argues that breastfeeding patterns have a strong impact on the linearity, resulting in increasing mortality after weaning. The curve for Sweden 1910--1946 show, however, no such downward bend [@bp51b, Figure 9; @bp52].
...
out[3, 2] <- "girl"
out[4, 3] <- 28
tbl(out, caption = "First five rows of data frame.")
nro <- nrow(infant)
...
```{r plotcohort, fig.cap = "By birth month, cohorts.", fig.height = 9, eval = FALSE}
oldpar <- par(mfrow = c(4, 3))
for (mon in levels(dat$bmonth)){
    drawBB2(dat[dat$bmonth == mon, ], main = mon, printLegend = FALSE)
}
par(oldpar)
```

```{r plotcohort2, fig.cap = "By birth season, cohorts.", fig.height = 8}
...
less convex (bend upward at the end) for those born in spring or summer, while 
the curves are concave for births in the fall or winter. Bad fits to "constant hazard"
and "uniform distribution".

### Periods by season

```{r plotperiod, fig.cap = "By calendar month, periods.", fig.height = 9, eval = FALSE}
oldpar <- par(mfrow = c(4, 3))
for (mon in levels(dat$bmonth)){
...
```{r plotperiod2, fig.cap = "By calendar season, periods.", fig.height = 8}
oldpar <- par(mfrow = c(2, 2))
...
and it shows off in that in all four periods the suggested models fit well, here shown by the
"constant hazards model" for exogenous mortality.

### Conclusion

The assumptions about constant hazard and uniform distribution of exogenous mortality 
are only applicable on population level, it can never hold for an individual. It can 
be thought of as an average over birth cohorts, where   
...
However, this data set is known to be of very high quality [@tbcl99], so we are inclined to 
